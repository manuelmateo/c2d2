// from: https://github.com/yekyam/YAGP/tree/main
#pragma once
...
class Args {
	/*
	 * \fn	auto find_arg(const std::vector<std::string_view> &args,
	 * std::string_view option_name) const noexcept
	...
	/*
	 * \fn std::optional<std::string_view> get_arg_value(std::string_view
	 * option_name) const noexcept
	 * \brief Attempts to get the `option_name`'s value in args ('-option_name
	 * value')
	 * \param std::string_view option_name The name of the option
	 * \return An std::optional containing the value or nullopt
	 */
	std::optional<std::string_view>
	get_arg_value(std::string_view option_name) const noexcept {
  ...
  public:
	 ...
	 * \note Assert is used here because argument parsing is the first thing in
	 * any program and defines program behavior. An error state here should in
	 * fact fail early.
	 */
	Args(int argc, char** argv, size_t max = 10) {
		assert(argc <= max &&
			   "Args error: argc greater than max (default is 10)");
		args = std::vector<std::string_view>(argv + 1, argv + argc);
	}

	/*
	 * \fn std::optional<T> get(std::string_view option_name, bool is_verbose =
	 ...
	 */
	template <typename T>
	std::optional<T> get(std::string_view option_name,
						 bool is_verbose = false) const noexcept;

	/*
	 * \fn std::optional<int> get(std::string_view option_name, bool is_verbose
	 * = false) const noexcept;
	 ...
	 */
	template <>
	std::optional<int> get<int>(std::string_view option_name,
		...
		int int_value{};
		auto [p, res] = std::from_chars(
			str_view_value.data(),
			str_view_value.data() + str_view_value.size(), int_value);
		if (res == std::errc{})
			return int_value;
		return std::nullopt;
	}
	...
	/*
	 * \fn std::optional<float> get(std::string_view option_name, bool
	 * is_verbose = false) const noexcept;
	 ...
	 */
	template <>
	std::optional<float> get<float>(std::string_view option_name,
									bool is_verbose) const noexcept {
		...
		if (str_view_value.data() == end) {
			if (is_verbose)
				std::cout << "Couldn't convert value " << str_view_value
						  << " to float!\n";
			return std::nullopt;
		}
		return f_value;
	...
	/*
	 * \fn std::optional<bool> get(std::string_view option_name, bool is_verbose
	 * = false) const noexcept;
	 ...
	 */
	template <>
	std::optional<bool> get<bool>(std::string_view option_name,
								  bool is_verbose) const noexcept {
	...
	/*
	 * \fn std::optional<std::string_view> get(std::string_view option_name,
	 * bool is_verbose = false) const noexcept; \brief Attempts to get the int
	 * value specified by option_name \param std::string_view option_name The
	 * name of the option \param bool is_verbose The verbosity of the function
	 * \return An std::optional containing the std::string_view value or nullopt
	 */
	template <>
	std::optional<std::string_view>
	get<std::string_view>(std::string_view option_name,
						  bool is_verbose) const noexcept {
